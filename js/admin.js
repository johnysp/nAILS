// Configuración de Firebase (usar la misma que en tu proyecto)
...
loginButton.addEventListener('click', async () => {
    ...
    try {
        ...
        console.log("UID del usuario que intenta loguearse:", user.uid); // <-- AÑADE ESTA LÍNEA
        ...
        if (isAdmin) {
            ...
            loadBookings();
            loadUsersAndDebts(); // ¡NUEVA FUNCIÓN!
...
[logoutButton, logoutButtonNav].forEach(btn => {
    btn.addEventListener('click', async () => {
        try {
            await auth.signOut();
            // showMessage(loginMessageDiv, 'Sesión cerrada correctamente.', 'success'); // Esto se manejará en onAuthStateChanged
        } catch (error) {
            // showMessage(loginMessageDiv, 'Error al cerrar sesión.', 'error'); // Esto se manejará en onAuthStateChanged
    ...
    if (user) {
        ...
        try {
            ...
            if (isAdmin) {
                ...
                userNavSection.style.display = 'flex'; // Muestra la sección de usuario en la navbar
                ...
                if (bookingsList.innerHTML.includes('Cargando citas...') || (debtsTableBody && debtsTableBody.innerHTML.includes('Cargando clientes...'))) {
                   loadBookings();
                   loadUsersAndDebts();
                }
            } else {
        ...
        userNavSection.style.display = 'none'; // Oculta la sección de usuario en la navbar
        adminEmailInput.value = '';
        adminPasswordInput.value = '';
        userEmailDisplay.textContent = '';
        userNavEmail.textContent = '';
        showMessage(loginMessageDiv, 'Has cerrado sesión o tu sesión ha expirado.', 'info'); // Mensaje informativo
    }
});
...
function loadUsersAndDebts() {
    ...
    usersRef.on('value', (snapshot) => {
        debtsTableBody.innerHTML = ''; // Limpiar la tabla
        let hasUsers = false;
        ...
        snapshot.forEach((childSnapshot) => {
            ...
            row.dataset.userId = userId; // Para facilitar la referencia
...
async function updateUserAdeudo(userId, newAdeudo) {
    try {
        // La seguridad de si el usuario es admin para escribir ya está en las reglas de Firebase.
        // Aquí solo hacemos la operación de escritura.
